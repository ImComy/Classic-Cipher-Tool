...
export const AnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
        ...
        {activeWorkspace === 'lab' && (
          <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-md shrink-0">
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded ${labTarget === 'source' ? 'bg-white text-primary-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
              Ciphertext
            </button>
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded ${labTarget === 'preview' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
              Live Preview
            </button>
      ...
      <FrequencyChart analysis={analysis} text={textToAnalyze} />
    </Modal>
  )
}