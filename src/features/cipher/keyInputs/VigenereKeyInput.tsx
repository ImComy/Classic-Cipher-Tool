...
export const VigenereKeyInput: React.FC = () => {
  const dispatch = useAppDispatch()
  const keyData = useAppSelector(state => state.cipher.keyData)
  const { toast } = useToast()

  const rawKey = keyData.key ?? 'KEY'
  ...
  const handleRandom = () => {
    ...
    toast('Random Vigenère keyword generated.')
  }
  ...
  return (
    <div className="w-full flex items-center justify-between gap-3 flex-wrap">
      ...
      <div className="flex-1 min-w-[100px] text-right sm:text-left">
        <span className="font-normal text-xs text-gray-400">Cᵢ = (Pᵢ + Kᵢ) mod 26</span>
      </div>
    </div>
  )
}
